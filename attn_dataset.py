...
class ATTNDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            ...
            #TODO: FORCE FOR ONLY ECHO TOP
            for i in range(len(self.weather_cube[0])):
                for j in range(len(self.products)):
                    if self.products[j] in wc[i].variables.keys():
                        wc_data['data'] = self.transform(wc[i][self.products[j]][:].reshape(-1,20,20), device=self.device)
                        wc_data['lats'] = self.transform(wc[i]['latitude'][:], device=self.device)
                        wc_data['lons'] = self.transform(wc[i]['longitude'][:], device=self.device)

        maxlen = min(len(fp), len(ft), min([len(wc_data[x]) for x in wc_data]))
        wc_data, fp, ft = {x:wc_data[x][:maxlen] for x in wc_data}, fp[:maxlen], ft[:maxlen]
...
def pad_batch(batch):
    ...
    cube_width = wc[0]['lats'].shape[-2]

    # extract weather cube items to separate variables
    wc_lats = [x['lats'] for x in wc]
    wc_lons = [x['lons'] for x in wc]
    wc_data = [x['data'] for x in wc]

    #pad fp, ft, wc
    fp, ft = pad_sequence(fp), pad_sequence(ft)
    wc = {'lats': pad_sequence(wc_lats), 'lons': pad_sequence(wc_lons), 'data': pad_sequence(wc_data)}
    #re-shape wc
